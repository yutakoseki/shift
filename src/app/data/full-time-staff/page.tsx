...
export default function FullTimeStaffPage() {
  ...
  const draftRows = data.fullTimeStaff
    .map((staff, index) => ({ staff, index }))
    .filter(({ staff }) => !persistedIds.includes(staff.id));
  const persistedRows = data.fullTimeStaff
    .map((staff, index) => ({ staff, index }))
    .filter(({ staff }) => persistedIds.includes(staff.id));
  ...
  function renderStaffRow(staff: FullTimeStaff, index: number): JSX.Element {
    ...
    return (
      <div key={staff.id} className="grid gap-2 rounded-md bg-orange-50 p-2 md:grid-cols-12">
        <input
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            {!isReadOnly && !classPickerOpenById[staff.id] ? (
              <button
                ...
                disabled={isReadOnly || classOptions.length === 0 || availableClassOptions.length === 0}
                onClick={() => {
          ...
          {!isReadOnly && classPickerOpenById[staff.id] ? (
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                disabled={isReadOnly || classOptions.length === 0 || availableClassOptions.length === 0 || !pendingClass}
                onClick={() => {
        ...
        <div className="flex flex-col gap-0.5 text-xs text-orange-900 md:col-span-5">
          <div className="flex items-center justify-between gap-2">
            <span>可能シフトパターン</span>
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {data.shiftPatterns.length === 0 ? (
                  ...
                  availableShiftOptions.map((pattern) => (
                      ...
                      ? {
                          ...
                          fullTimeStaff: [
                            ...prev.fullTimeStaff,
                            {
                              ...
                              name: "",
                              mainClass: "",
                              possibleShiftPatternCodes: nonCustomShiftPatternCodes
                            }
                          ]
                        }
                      : prev
